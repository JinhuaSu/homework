...
//sample Gaussian-destributed random numbers via Box-Muller algorithm
...
//free a data matrix
void delete2Darray(double **tmp){
    free(tmp[0]);
    free(tmp);
}

//compute distance between data and centers 
...
int* compute_Z(double **dat, int n_dat, int p, double **center){
    ...
    for(int i=0; i<n_dat; i++){
        for(int k=0; k<K; k++){
            dist_tmp = 0;
            for(int j=0; j<p; j++){
                dist_tmp += (dat[i][j] - center[k][j])*(dat[i][j] - center[k][j]);
            }
            if(k==0){
                dist = dist_tmp;
                printf("%f\n",dist);
...
int main(){
    ...
    if(world_rank == 0){
        MPI_Send(dat[(world_size-1)*(N/world_size)], p*(N - (world_size - 1) *(N/world_size)),MPI_DOUBLE,world_size-1,888,MPI_COMM_WORLD);
    }

    if(world_rank == world_size -1){
        MPI_Recv(dat_partial[0],p*(N-(world_size-1)*(N/world_size)),MPI_DOUBLE,0,888,MPI_COMM_WORLD,MPI_STATUS_IGNORE);
    }
    ...
    for(t=0;t<5;t++){
        ...
        if(world_rank == 0){
            MPI_Recv(&(Z[(world_size-1)*(N/world_size)]),N-(world_size-1)*(N/world_size), MPI_INT, world_size-1, 888, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
        }

        //step2: compute centers on pressor 0
        if(world_rank == 0){
            d = compute_d(dat, p , Z);
            printf("========================Iteration %d==================\n",t);
            printf("cluster 1's center is (%f,%f)\n",d[0][0],d[0][1]);
